...
module.exports.parseTimerCommand = function(inputString) {
  ...
  if (withKvs.in) {
    ...
    var match  = withKvs.at.match(/(\d{1,2}):(\d{1,2})/)
    if (match) {
      withKvs.notes = [];
      withKvs.executionTime =  moment.tz(withKvs.at, "hh:mm" , "Europe/Amsterdam").toDate();
    } else {
      withKvs.text = "";
      withKvs.error = "Unrecognized at string: " + withKvs.at;
      withKvs.notes = ["Could not parse " + withKvs.at + " within \"" + inputString + "\"" ];
    }
  } else {
