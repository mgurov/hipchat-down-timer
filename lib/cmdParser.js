module.exports.parseTimerCommand = function(inputString) {
  ...
  if (withKvs.in) {
    ...
    var match  = withKvs.at.match(/(\d{1,2}):(\d:2)/)
    if (match) {
      withKvs.executionTime.setHours(parseInt(match[0]));
      withKvs.executionTime.setMinutes(parseInt(match[1]));
      withKvs.executionTime.setSeconds(0);
      withKvs.executionTime.setMilliseconds(0);
    } else {
      withKvs.text = "";
      withKvs.error = "Unrecognized at string: " + withKvs.at;
      withKvs.notes = ["Could not parse input \"" + inputString + "\"" ];
    }
  } else {
