...
describe('command parsing e2e', function() {
  ...
  it('local time before now means tomorrow', function() {
    ...
    if (now.getHours() <= 0 && now.getMinutes() <= 2) {
      return; //two minutes a day ignore this test
    }

    var actual = parser.parseTimerCommand("sample text at=0:01").executionTime;
    //assert.equal(new Date(), actual);
    assert.deepEqual({h: 0, m: 1}, {h: actual.getHours(), m: actual.getMinutes()});
    assert.isTrue(now.getTime() < actual.getTime()) //and execution time is in the future.
  });

  it('local time after now means today', function() {
    var now = new Date();
    if (now.getHours() >= 23 && now.getMinutes() >= 58) {
      return; //two minutes a day ignore this test
    }
    var actual = parser.parseTimerCommand("sample text at=23:59").executionTime;
    //assert.equal(new Date(), actual);
    assert.deepEqual({d: now.getDate(), h: 23, m: 59}, {d: actual.getDate(), h: actual.getHours(), m: actual.getMinutes()});
  });
