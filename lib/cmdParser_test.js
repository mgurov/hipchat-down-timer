...
describe('command parsing e2e', function() {
  ...
  it('understands local time', function() {
    var actual = parser.parseTimerCommand("sample text at=8:06");
    assert.deepEqual([], actual.notes);
    assert.equalTime(actual.executionTime, new Date(Date.UTC(2006, 01, 02, 8, 6, 0, 0)));
  });

  xit('understands local time with leading zero', function() {
    var actual = parser.parseTimerCommand("sample text at=08:06");
    assert.equalTime(actual.executionTime, new Date(Date.UTC(2006, 01, 02, 8, 6, 0, 0)));
  });

  xit('understands local time with leading zero', function() {
    var actual = parser.parseTimerCommand("sample text at=18:06");
    assert.equalTime(actual.executionTime, new Date(Date.UTC(2006, 01, 02, 18, 6, 0, 0)));
  });
